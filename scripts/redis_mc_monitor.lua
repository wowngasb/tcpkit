...
function calculate_request_cost(key, item)
    if packet_hash_cmd[key] then
        ...
        if string.len(packet_hash_cmd[key]) > 80 then
            print(string.format("%s | %36s | %1.80s... | %3.3fms", time_str, key, packet_hash_cmd[key], cost))
        else
            print(string.format("%s | %36s | %-83s | %3.3fms", time_str, key, packet_hash_cmd[key], cost))
        end
